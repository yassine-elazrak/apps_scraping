...
class  load_visual:
    ...
    def show_word(self):
        if not 'clean_tweet' in self.df.columns:
            showerror("Error Visual word", "not find column clean_tweet, please clean file ")
            return
    ...
    def sentiment(self):
        if not 'clean_tweet' in self.df.columns:
            showerror("Error Visual sentiment :", "not find column clean_tweet, please clean file ")
            return
    ...
    def cluster(self):
        if not 'clean_tweet' in self.df.columns:
            showerror("Error Visual cluster :", "not find column clean_tweet, please clean file ")
            return
        self.df['pca'] = (
            self.df['clean_tweet']
...
class  Visual:
    ...
    def set_dict(self , dict):
        self.dict_clean = dict


    def exec(self):
        if not self.dir or not os.path.exists(self.dir):
            showerror("Error ", "not find file")# '%s'" % Path(self.dir).name)
            return

        for widget in self.parent.winfo_children():
            widget.destroy()
        ################################################################
        load_visual(self.parent  , self.dir , self.var_clean).controller()
